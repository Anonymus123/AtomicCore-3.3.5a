...
CalendarEvent* CalendarMgr::GetEvent(uint64 eventId) const
{
    ...
    sLog->outStaticDebug("CalendarMgr::GetEvent: [" UI64FMTD "] not found!", eventId);
    return NULL;
}
...
CalendarInvite* CalendarMgr::GetInvite(uint64 inviteId) const
{
    ...
    sLog->outStaticDebug("CalendarMgr::GetInvite: [" UI64FMTD "] not found!", inviteId);
    return NULL;
}
...
std::string CalendarEvent::BuildCalendarMailBody() const
{
    ...
    data << uint32(secsToTimeBitFields(_eventTime));
    data >> time;
    strm << time;
    return strm.str();
}
...
void CalendarMgr::SendCalendarEventInvite(CalendarInvite const& invite)
{
    ...
    if (hasStatusTime)
        data << uint32(secsToTimeBitFields(statusTime));
...
void CalendarMgr::SendCalendarEventUpdateAlert(CalendarEvent const& calendarEvent, time_t oldEventTime)
{
    ...
    data << uint32(secsToTimeBitFields(oldEventTime));
    data << uint32(calendarEvent.GetFlags());
    data << uint32(secsToTimeBitFields(calendarEvent.GetEventTime()));
...
void CalendarMgr::SendCalendarEventStatus(CalendarEvent const& calendarEvent, CalendarInvite const& invite)
{
    ...
    data << uint32(secsToTimeBitFields(invite.GetStatusTime()));
...
void CalendarMgr::SendCalendarEvent(uint64 guid, CalendarEvent const& calendarEvent, CalendarSendEventType sendType)
{
    ...
    data << uint32(calendarEvent.GetFlags());
    data << uint32(secsToTimeBitFields(calendarEvent.GetEventTime()));
    data << uint32(secsToTimeBitFields(calendarEvent.GetTimeZoneTime()));
    ...
    for (CalendarInviteStore::const_iterator itr = eventInviteeList.begin(); itr != eventInviteeList.end(); ++itr)
    {
        ...
        data << uint32(secsToTimeBitFields(calendarInvite->GetStatusTime()));
