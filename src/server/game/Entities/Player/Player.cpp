...
void Player::Update(uint32 p_time)
{
    ...
    if (m_deathTimer > 0 && !GetBaseMap()->Instanceable() && !HasAuraType(SPELL_AURA_PREVENT_RESURRECTION) && getDeathState() != GHOULED)
    {
    ...
    if (!(currFields & val))
    {
        ...
        if (areaEntry->area_level > 0)
        {
            ...
            else
            {
				...
				XP = uint32(XP * (IsEventActive(sWorld->getIntConfig(CONFIG_RATE_XP_WEEKEND_EVID)) ? sWorld->getRate(RATE_XP_WEEKEND_EXPLORE) : 1.0f));
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    uint32 XP = rewarded ? 0 : uint32(quest->XPValue(this)*sWorld->getRate(RATE_XP_QUEST) * (IsEventActive(sWorld->getIntConfig(CONFIG_RATE_XP_WEEKEND_EVID)) ? sWorld->getRate(RATE_XP_WEEKEND_QUEST) : 1.0f));
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    {
        if(AreaTrigger const* pAT = sObjectMgr->GetMapEntranceTrigger(mapId))
            Relocate(pAT->target_X, pAT->target_Y, pAT->target_Z, pAT->target_Orientation);
        else
        {
...
void Player::CreateWowarmoryFeed(uint32 type, uint32 data, uint32 item_guid, uint32 item_quality) {
    ...
    */
    if (GetGUIDLow() == 0)
    {
        sLog->outErrorDb("[Wowarmory]: player is not initialized, unable to create log entry!");
        return;
    }
    if (type <= 0 || type > 3)
    {
        sLog->outErrorDb("[Wowarmory]: unknown feed type: %d, ignore.", type);
        return;
    }
    if (data == 0)
    {
        sLog->outErrorDb("[Wowarmory]: empty data (GUID: %u), ignore.", GetGUIDLow());
        return;
    }
