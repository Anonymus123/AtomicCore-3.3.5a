...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recvData)
{
    ...
    if (action == 1 && ginfo.ArenaType == 0)
    {
        ...
        if (!_player->CanJoinToBattleground())
        {
            ...
            action = 0;
            sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Player %s (%u) has a deserter debuff, do not port him to battleground!", _player->GetName(), _player->GetGUIDLow());
        }
        //if player don't match battleground max level, then do not allow him to enter! (this might happen when player leveled up during his waiting in queue
        if (_player->getLevel() > bg->GetMaxLevel())
        {
            sLog->outError("Player %s (%u) has level (%u) higher than maxlevel (%u) of battleground (%u)! Do not port him to battleground!",
