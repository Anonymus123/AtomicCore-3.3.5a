        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_ROGUE:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[EFFECT_1] & 0x8)
                {
                    ...
                    for (Unit::AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
                        ...
                        {
                            uint32 chance = GetSpellInfo()->Effects[EFFECT_2].CalcValue();
            ...
            case SPELLFAMILY_PALADIN:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1]&0x00040000)
                {
					...
					{
						float average = (m_caster->GetFloatValue(UNIT_FIELD_MINDAMAGE) + m_caster->GetFloatValue(UNIT_FIELD_MAXDAMAGE));
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            switch (m_spellInfo->Id)
            {
                ...
                case 61999:
                    ...
                    {
                        SendCastResult(SPELL_FAILED_TARGET_NOT_PLAYER);
                        finish(true);
                        CancelGlobalCooldown();
                        m_caster->ToPlayer()->RemoveSpellCooldown(m_spellInfo->Id, true);
                        return;
            ...
            switch (m_spellInfo->Id)
            {
                ...
                case 54171:                                   //Divine Storm
                {
                    ...
                    if (m_UniqueTargetInfo.size())
                    {
                        ...
                        int32 heal = GetSpellInfo()->Effects[EFFECT_1].CalcValue() * damage / m_UniqueTargetInfo.size() / 100;
...
void Spell::EffectCastButtons(SpellEffIndex effIndex)
{
    ...
    for (; n_buttons; --n_buttons, ++button_id)
    {
        ...
        if (m_caster->GetPower(POWER_MANA) < cost)
         ...
         if (!m_caster->HasSpell(spell_id) || spellInfo->IsPassive())
            continue;
