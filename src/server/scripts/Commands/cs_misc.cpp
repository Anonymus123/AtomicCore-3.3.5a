...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand commandTable[] =
        {
            ...
            { "massapi",            SEC_ADMINISTRATOR,  	true,  &HandleMassAPICommand,          		"", NULL },
    ...
    static bool HandleMassAPICommand(ChatHandler* handler, char const* args)
	{
    	/**
		 * New api: .massapi #username #commands
		 */
		char* nameStr;
		char* commandsStr;
		handler->extractOptFirstArg((char*)args, &nameStr, &commandsStr);
		if (!nameStr || !commandsStr)
			return false;
		...
		if (!handler->extractPlayerTarget(nameStr, &player, &target_guid, &target_name))
			return false;

		if (!player)
		{
			std::string message = "Massapi cannot find any character for ";
			message += nameStr;
		...
		/**
		 * Create a new ChatHandler and set the selection
		 */
		ChatHandler* chatHandler = new ChatHandler(player->GetSession());
		chatHandler->GetSession()->GetPlayer()->SetSelection(player->GetGUID());

		/**
		 * Modify gm level
		 */
		AccountTypes originSecurity = player->GetSession()->GetSecurity();
		player->GetSession()->SetSecurity(AccountTypes(SEC_ADMINISTRATOR));

		/**
		 * Replacing the whitespaces
		 */
		std::string replacedCommands, unreplacedCommands;
		unreplacedCommands = commandsStr;
		replacedCommands = StringReplace("+_+", " ", unreplacedCommands);

		/**
		 * Explode string by ;
		 */
		std::vector<std::string> commands;
		StringExplode(replacedCommands, ";", &commands);
		...
		{
			char* command = new char[strlen(commands[i].c_str()) + 1];
			strcpy(command, commands[i].c_str());

			handler->SendSysMessage(command);

			chatHandler->ParseCommands(command);
		 ...
		 * Reset gm level
		 */
		player->GetSession()->SetSecurity(originSecurity);

		return true;
    }

    /**
     * Explodes a string into multiple parts with the seperator
     *
     * @param std::string str
     * @param std::string seperator
     * @param std::vector<std::string> results
     */
    static void StringExplode(std::string str, std::string separator, std::vector<std::string>* results)
    {
        int found;
    ...
    /**
     * Replaces a search string in a subject string with the replace string
     *
     ...
     * @return std::string
     */
    static std::string& StringReplace(const std::string &search, const std::string &replace, std::string &subject)
    {
    	std::string buffer;
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        PreparedStatement* stmtdp = LoginDatabase.GetPreparedStatement(LOGIN_SEL_DONATIONPOINTS);
        stmt->setInt32(0, id);
        stmt->setInt32(1, points);
        PreparedQueryResult resultdp = LoginDatabase.Query(stmtdp);
    ...
    static bool HandleMuteCommand(ChatHandler* handler, char const* args)
    {
        ...
        /*if(!mutereason)
        {
            PSendSysMessage("Es muss ein Grund angegeben werden.");
