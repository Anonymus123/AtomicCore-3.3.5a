       ...
       void DamageTaken(Unit* pDoneBy, uint32 &uiDamage)
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (m_bIsDuelInProgress)
            {
                ...
                {
                    me->getVictim()->CastSpell(me->getVictim(), 7267, true);
...
public:
    ...
    struct npc_scarlet_miner_cartAI : public PassiveAI
    {
        ...
        void DoAction(const int32 /*param*/)
        {
            ...
            {
				me->GetPlayer(*me, me->GetCreatorGUID())->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                me->SetWalk(false);
            ...
            switch (waypointId)
            {
                ...
                case 17:
                    if (Unit* car = Unit::GetCreature(*me, carGUID))
                    {
						...
						car->GetPlayer(*me, car->GetCreatorGUID())->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                    }
            ...
            if (Creature* miner = player->SummonCreature(NPC_SCARLET_MINER, 2383.869629f, -5900.312500f, 107.996086f, player->GetOrientation(), TEMPSUMMON_DEAD_DESPAWN, 1))
            {
                ...
                if (Creature* car = player->GetVehicleCreatureBase())
                {
                    if (car->GetEntry() == NPC_MINE_CAR)
                    {
						...
						car->GetPlayer(*car, car->GetCreatorGUID())->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        CAST_AI(npc_scarlet_miner::npc_scarlet_minerAI, miner->AI())->InitCartQuest(player);
                    } else sLog->outError("OnGossipHello vehicle entry is not correct.");
                } else sLog->outError("OnGossipHello player is not on the vehicle.");
            } else sLog->outError("OnGossipHello Scarlet Miner cant be found by script.");
        }
        return true;
    }
