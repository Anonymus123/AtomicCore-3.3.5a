...
class npc_high_overlord_saurfang_icc : public CreatureScript
{
    public:
            ...
            void DoAction(int32 const action)
            {
                switch (action)
                {
                    ...
                    case ACTION_START_OUTRO:
                    {
                        ...
                        _events.ScheduleEvent(EVENT_OUTRO_HORDE_2, 10000);
                        _events.ScheduleEvent(EVENT_OUTRO_HORDE_3, 18000);
                        _events.ScheduleEvent(EVENT_OUTRO_HORDE_4, 24000);
                        _events.ScheduleEvent(EVENT_OUTRO_HORDE_5, 30000);
                ...
                if (type == POINT_MOTION_TYPE)
                {
                    switch (id)
                    {
                        ...
                        case POINT_CORPSE:
                            ...
                            _events.ScheduleEvent(EVENT_OUTRO_HORDE_5, 1000);
                            _events.ScheduleEvent(EVENT_OUTRO_HORDE_6, 4000);
                            break;
                        case POINT_FINAL:
                            if (Creature* deathbringer = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_DEATHBRINGER_SAURFANG)))
        ...
        bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Wir sind bereit, Hochfuerst. Der Lich Koenig muss fallen!", 631, -ACTION_START_EVENT);
...
class npc_muradin_bronzebeard_icc : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            if (instance && instance->GetBossState(DATA_DEATHBRINGER_SAURFANG) != DONE)
            {
                player->ADD_GOSSIP_ITEM(0, "Lasst uns beginnen...", 631, -ACTION_START_EVENT + 1);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            }
