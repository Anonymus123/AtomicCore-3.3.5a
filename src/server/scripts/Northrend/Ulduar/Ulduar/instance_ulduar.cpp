            ...
            bool SetBossState(uint32 type, EncounterState state)
            {
                ...
                switch (type)
                {
                    ...
                    case BOSS_ALGALON:
                        if (state == IN_PROGRESS)
                        {
                            HandleGameObject(AlgalonDoorGUID, false);
                            HandleGameObject(AlgalonFloorOOCGUID, true);
                            HandleGameObject(AlgalonFloorCOMGUID, false);
                            HandleGameObject(AlgalonBridgeGUID, true);
                            HandleGameObject(AlgalonGlobeGUID, true);
                            HandleGameObject(AlgalonForceFieldGUID, false);
                        }
                        else
                        {
