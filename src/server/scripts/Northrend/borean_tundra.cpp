...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (player->GetQuestStatus(QUEST_ACES_HIGH) == QUEST_STATUS_INCOMPLETE || player->GetQuestStatus(QUEST_ACES_HIGH_DAILY) == QUEST_STATUS_INCOMPLETE)
    ...
    struct npc_seaforium_depth_chargeAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            if (uiExplosionTimer < diff)
            {
                ...
                for (uint8 i = 0; i < 4; ++i)
                {
                    if (Creature* cCredit = me->FindNearestCreature(25402 + i, 10.0f))
                    {
